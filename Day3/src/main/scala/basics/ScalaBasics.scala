...
object ScalaBasics {
  ...
  def minRecursive(r: Array[Int]): Int = {
    if(r.length == 1){
      ...
      if(r(0)> r(r.length -1)) {
        minRecursive(r.slice(1,r.length))
      }else{
        minRecursive(r.slice(0,r.length -1))
      }
    }
  }
  ...
  def isPalindrome(s: String): Boolean = {
    if((for (e <- s.toCharArray) yield e.toLower).mkString("").
      replaceAll("[.?,;!'. ]","").replaceAll("-","").
      equals(s.toLowerCase().replaceAll("[.?,;!'. ]","").
        replaceAll("-","").reverse)){
        true
    }else{
      false
