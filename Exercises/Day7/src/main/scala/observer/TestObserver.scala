...
object TestObserver extends App {
    ...
    val observer: Observer = SMSUsers(subject, "Adam Warner [New York]")
    observer.subscribe()
    println()

    val observer2: Observer = SMSUsers(subject, "Tim Ronney [London]")
    observer2.subscribe()
    ...
    val observer3: Observer = SMSUsers(subject, "Marrie [Paris]")
    observer3.subscribe()
    println()
